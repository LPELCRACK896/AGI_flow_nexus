...
class Config:
    def __init__(self, env_path, required_fields=[]) -> None:
    ...
    def setup(self):
        ...
        raise ValueError(f"Could not find required fields: {','.join(missing_fields)} in given enf file ({self.__env_path})")
        
    def __get_envs(self):
        if os.path.exists(self.__env_path):
            return dotenv_values(self.__env_path)  
        raise FileNotFoundError(f"The environment file at {self.__env_path} was not found. TIP: your current path is {os.getcwd()}")
