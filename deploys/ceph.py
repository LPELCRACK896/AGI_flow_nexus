...
class CephConnection:
    ...
    @task(name="Upload image to ceph")
    def upload_satellite_image(self, file_data: bytes, file_date: date, area: str, product: str):
        logger = get_run_logger()
        filename = build_file_name(file_date, area, product)
        target_bucket = (
    ...
    def __upload_data(self, filename: str, file_data: bytes, bucket: str):
        try:
            # Subir el archivo desde memoria usando BytesIO
            file_obj = BytesIO(file_data)
            response = self.s3.upload_fileobj(file_obj, bucket, filename)

            # Obtener ETag del archivo subido
            etag = self.s3.head_object(Bucket=bucket, Key=filename)["ETag"]
            print(f"File '{filename}' successfully uploaded to bucket '{bucket}'. ETag: {etag}")
            return etag
        except Exception as e:
            print(f"An error occurred uploading s3 file: {e}")
            return None
